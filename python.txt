...
E) Modulo Division
F) Integer division
G) Exponentiation (Âîçâåäåíèå â ñòåïåíü)
...
$$$ 39
Îöåíî÷íàÿ ëîãèêà â Python 3. Logical operators that have a value of a logical (Boolean) type, i.e. True or False: 
A) Equality
B) Inequality
C) More
+D) Logical AND
...
$$$ 46
Python.  Instructions, if the condition is true, then the block of expressions “if-block” is executed, otherwise another block of expressions “else-block” is executed:
A) Control flow   
B) Loop 
+C) if – elif - else
D) for 
...
$$$ 71
Python. The type of arguments that are a «name-value» pair passed to the function:
A) Positional Arguments
+B) Named Arguments
C) String
D) Floating point numbers
...
$$$ 73
Python. Data structures, which are a collection of «key-value» pairs. Each key is associated with a certain value, and the program can get the value associated with the given key:
A) Positional Arguments
+B) Dictionary
C) String
D) Floating point numbers
...
+A) Êîìàíäíàÿ ñòðîêà (command line) 
B) Êîììåíòàðèé (comment) 
C) Êîìïèëÿòîð (compiler) 
D) Ëèòåðàë (literal)
E) Èäåíòèôèêàòîð (identifier) 
F) Èíòåðïðåòàòîð (interpreter) 
G) Èñõîäíûé êîä (source code) 
...
A) Integer (Öåëûå ÷èñëà).
B) Float (Âåùåñòâåííûå ÷èñëà). 
C) String (Ñòðîêà)
+D) List (Ñïèñîê).  
E) Tuple (Êîðòåæ). 
F) Set (Ìíîæåñòâî).  
G) Dictionary (Ñëîâàðü).
H) Logical (Ëîãè÷åñêèå çíà÷åíèÿ).

$$$ 102
Immutable (íåèçìåíÿåìûå) Lists:  
A) Integer (Öåëûå ÷èñëà).
B) Float (Âåùåñòâåííûå ÷èñëà). 
C) String (Ñòðîêà)
D) List (Ñïèñîê).  
...
$$$ 111
Instructions for finding a slice (ñðåç) of a list that contains elements of the list (list) with numbers from i to j in increments of k:  
A) list = [ 'January' , 'February' , 'March' ] ,3
B) print(list[0] ) 
C) print( '\nTop Left 0,0 :' , list[1][0] )
D) list = [ [ 1 , 2 , 3 ] , [ 4 , 5 , 6 ] ]
...
$$$ 121
A set method that returns elements from the set set1 that are not in set2 (ðàçíîñòü ìíîæåñòâ  set1 - set2):
A) set.add (x)  
B) set.update (x, y, z) 	
C) set.pop ()		
D) set1.intersection (set2) 
...
$$$ 123
A statement that checks for a condition that evaluates a given expression: True èëè False:  
+A) if: 
B) else:
C) break
D) for…in
...
if n % 2 == 0:
print('Ýòî ÷èñëî ÷åòíîå')
else:
print('Ýòî ÷èñëî íå÷åòíîå')
A) ÷èñëî óâåëè÷èòü â 2 ðàçà
B) ÷èñëî óìåíüøèòü â 2 ðàçà
C) ýòî ÷èñëî ÷åòíîå
+D) ýòî ÷èñëî íå÷åòíîå 
E) n=5
...
$$$ 130
Variable value  ñ after program execution:
a = 40
b = 10
b = –à/2 * b
if a < b:
...
$$$ 140
Operators to output the real value of a variable x = -2.12 (íàèáîëåå îïòèìàëüíûé):
A) ðrint( 'x=5' ) 
B) ðrint (5) 
+C) ðrint (‘% 5.2f ‘% x) 
D) ðrint (‘% .3f ‘% x) 
...
$$$ 157
The operation to find the slice (ñðåç) of the lst list:
A) len (lst) 
B) lst ? n
+C) lst [i:j:k]
D) min (lst)
...
$$$ 179
The result of the execution of the program fragment  (values i è j):
for i in  range (1, 2):
print( i )
for j in  range (1, 3):
     print( j )
...
$$$ 180
The result of the execution of the program fragment  (values s è s1):
s=0
s1=0
i = 1
while i<4 :
...
$$$ 191
Methods for determining (óïàêîâêè) a tuple:
+A) k = ( 'Red' , 'Green' , 'Red' , 'Blue' ) 
B) ‘color’ 
+C) pr = (200, 50)  
D) s1 = {'cat': 'êîøêà', 'dog': 'ñîáàêà', 'mouse': 'ìûøü'} 
E) [2, 3]
F) s1 = set ([3,6,3,5])
...
B) ‘color’ 
C) (200, 50) 
D) {'cat': 'êîøêà', 'dog': 'ñîáàêà', 'mouse': 'ìûøü'} 
E) [2, 3]
F) {3, 6, 3, 5}  
...
B) ‘color’ 
C) pr = (200, 50) 
+D) d = {'cat': 'êîøêà', 'dog': 'ñîáàêà', 'mouse': 'ìûøü'} 
E) [2, 3]
F) s2 = {3, 6, 3, 5}  
